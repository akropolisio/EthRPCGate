...
func TestCacheWithClient(t *testing.T) {
	kaonMockServer := NewKaonMockServer(mockJsonRPCResponse)
	URL := "http://kaonuser:kaonpass@127.0.0.1:6969"
	defer kaonMockServer.Close()
	...
	client.URL = kaonMockServer.URL
	if err != nil {
		t.Fatal(err)
	}

	var result interface{}
	t.Run("Kaon RPC getblock call processed succesfully", func(t *testing.T) {
	...
	t.Run("Upon second getblock call, response is returned from cache", func(t *testing.T) {
		...
		if !strings.Contains(outputLog, "kaon (CACHED) RPC response") {
			t.Errorf("\nexpected: %s\n\n, got: %s", "kaon (CACHED) RPC response", outputLog)
		}
